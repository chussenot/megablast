...
/// without that edit.
///
/// Wired in by `game::enemies`'s Boss (mb-epic.22): stashed on the Boss
/// enemy at spawn time and stamped onto every bullet it fires, so a
/// future render pass can pick the bullet color/palette per level.
